...
class BonitoStaticQuant(Module):
    """
    Model template for QuartzNet style architectures
    https://arxiv.org/pdf/1910.10261.pdf
    """
    def __init__(self, config):
        super(BonitoStaticQuant, self).__init__()
        ...
        self.quant_identity=qnn.QuantIdentity(
             bit_width=layer_weight_bits, return_quant_tensor=True)
    def forward(self, x):
        ...
        for layer in self.config['block']:
            encoder_layers.append(
                Block(
                    ...
                    quant=layer_weight_bits,quant_act=act_bits
                )
            )
...
class Block(Module):
    ...
    """
    def __init__(self, in_channels, out_channels, activation, repeat=5, kernel_size=1, stride=1, dilation=1, dropout=0.0, residual=False, separable=False, quant=16,quant_act=16):
        ...
        if self.use_res:
            self.residual = Sequential(*self.get_tcs_quant(in_channels, out_channels,quant=quant,quant_act=quant_act))
    ...
    def __init__(self, features, classes):
        ...
        self.layers = Sequential(
            qnn.QuantConv1d(features, classes, kernel_size=1, bias=True,
                bias_quant=None,
                return_quant_tensor=False,
                weight_bit_width=layer_weight_bits),
            Permute([2, 0, 1])
        )
